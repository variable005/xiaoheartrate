...
void loop() {
  ...
  // Heavy smoothing for ESP32-C3
  filtered = (filtered * 9 + rawSignal) / 10;
  baseline = (baseline * 49 + filtered) / 50;

  int threshold = baseline + 25;
  unsigned long now = millis();

  // Peak detection (one beat only)
  if (!inBeat && filtered > threshold && (now - lastBeatTime) > 650) {
    inBeat = true;
